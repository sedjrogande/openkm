...
public class IdentitySession implements ExpressionSession {
	...
	public Group getGroupByName(String name) {
		...
		try {
			...
			for (String user : CommonAuthModule.getUsersByRole(null, name)) {
	...
	public User getUserByName(String userName) {
		...
		try {
			...
			for (String role : CommonAuthModule.getRolesByUser(null, userName)) {
				Membership membership = new Membership();
	...
	public User getUserByGroupAndRole(String group, String role) {
		...
		try {
			for (String userId : CommonAuthModule.getUsersByRole(null, group)) {
